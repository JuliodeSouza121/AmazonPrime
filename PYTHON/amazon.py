'''
Dataset contendo informções sobre filmes e séries da Amazon Prime Video.
Cada Linha representa um título (filme ou série) disponível na plataforma.
Colunas:
- show_id: Identificador único do título.
- type: Tipo do título (filme ou série).
- title: Título do filme ou série.
- director: Diretor do título.
- cast: Elenco principal do título.
- country: País de origem do título.    
- date_added: Data em que o título foi adicionado à plataforma.
- release_year: Ano de lançamento do título.        
- rating: Classificação indicativa do título.
- duration: Duração do título (em minutos para filmes e em temporadas para séries).
- listed_in: Gêneros ou categorias do título.
- description: Breve descrição do título.


Fonte:
https://www.kaggle.com/datasets/shivamb/amazon-prime-movies-and-shows?resource=download
'''
import pandas as pd
import numpy as np
